...
def voxel(O, voxel_type):
    ...
    if voxel_type == VT.Air:
        ...
        mesh_box.paint_uniform_color([0.59, 0.27, 0.07])
    else:
...
class WinApp:
    ...
    def __init__(self, width, height):
        ...
        self.settings.new_ibl_name = resource_path + "/" + WinApp.DEFAULT_IBL
        ...
        self._ibl_map.selected_text = WinApp.DEFAULT_IBL
        ...
        self._shader.add_item(WinApp.MATERIAL_NAMES[0])
        self._shader.add_item(WinApp.MATERIAL_NAMES[1])
        self._shader.add_item(WinApp.MATERIAL_NAMES[2])
        self._shader.add_item(WinApp.MATERIAL_NAMES[3])
        ...
        if gui.Application.instance.menubar is None:
            if isMacOS:
                app_menu = gui.Menu()
                app_menu.add_item("About", WinApp.MENU_ABOUT)
                app_menu.add_separator()
                app_menu.add_item("Quit", WinApp.MENU_QUIT)
            file_menu = gui.Menu()
            file_menu.add_item("Open", WinApp.MENU_OPEN)
            file_menu.add_item("To Minecraft", WinApp.CUBE_MODE)
            if not isMacOS:
                file_menu.add_separator()
                file_menu.add_item("Quit", WinApp.MENU_QUIT)
            settings_menu = gui.Menu()
            settings_menu.add_item("Lighting & Materials",
                                   WinApp.MENU_SHOW_SETTINGS)
            settings_menu.set_checked(WinApp.MENU_SHOW_SETTINGS, True)
            help_menu = gui.Menu()
            help_menu.add_item("About", WinApp.MENU_ABOUT)
        ...
        w.set_on_menu_item_activated(WinApp.MENU_OPEN, self._on_menu_open)
        w.set_on_menu_item_activated(WinApp.CUBE_MODE,
                                     self._on_cube_mode)
        w.set_on_menu_item_activated(WinApp.MENU_QUIT, self._on_menu_quit)
        w.set_on_menu_item_activated(WinApp.MENU_SHOW_SETTINGS,
                                     self._on_menu_toggle_settings_panel)
        w.set_on_menu_item_activated(WinApp.MENU_ABOUT, self._on_menu_about)
        # ----
    ...
    def _on_shader(self, name, index):
        self.settings.set_material(WinApp.MATERIAL_SHADERS[index])
        self._apply_settings()
    ...
    def _on_cube_mode(self):
        print("Start! Building...")
        try:
            voxel_map = VoxelMap(self._cloud)
            print("Got cubes, segmentation...")
            model = CubeForestModel()
            model.add_dataset(voxel_map)
            model.load()
            viz = model.predict()

            print("Segmentation completed! Drawing...")
            mesh = o3d.geometry.TriangleMesh()

            for (k, j, i, category) in viz:
                mesh += voxel(np.array([k, j, i]), category)
    ...
    def _on_menu_toggle_settings_panel(self):
        ...
        gui.Application.instance.menubar.set_checked(
            WinApp.MENU_SHOW_SETTINGS, self._settings_panel.visible)
    ...
    def _on_menu_about(self):
        ...
        dlg_layout.add_child(gui.Label("Cube Forest, Yakubov A.R. IU5"))
    ...
    def load(self, path):
        ...
        self._cloud = np.asarray(cloud.points)
